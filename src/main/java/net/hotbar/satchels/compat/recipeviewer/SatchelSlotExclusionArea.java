 ...
 * both the JEI and EMI plugins so their recipe grid overlay doesn't cover the slot — in any
 * allowed menu whose {@link SatchelEquipmentSlot} was added by {@code AbstractContainerMenuMixin}
 * / {@code InventoryMenuMixin} / {@code HorseInventoryMenuMixin}, not just {@code InventoryScreen}.
 * Generic over any menu type ({@code <?>}) rather than {@code InventoryMenu} specifically, since
 * nothing here actually needs an {@code InventoryMenu}-specific method — it just looks for a
